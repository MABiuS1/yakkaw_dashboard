...
import { Category, CategoryForm } from "@/constant/categoryData";
...
export const useCategories = () => {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      setIsUpdating(true);
      const res = await fetch(`${API_BASE_URL}/admin/categories/${editingId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify(currentForm),
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      const res = await fetch(`${API_BASE_URL}/admin/categories/${categoryToDelete}`, {
        method: "DELETE",
        credentials: "include",
      });
